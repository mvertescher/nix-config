...
#[derive(Debug, Clone, Copy)]
pub struct InfoPanelBackground {
    pub bg_color: Color,
    pub border_color: Color,
    pub cut_size: f32,
    pub border_width: f32,
...
impl<Message> canvas::Program<Message> for InfoPanelBackground {
    type State = ();
    ...
    fn draw(
        ...
        if w > 2.0 * cut && h > 2.0 * cut {
            let path = canvas::Path::new(|builder| {
                builder.move_to(Point::new(0.0, 0.0));
                builder.line_to(Point::new(w - cut, 0.0));
                builder.line_to(Point::new(w, cut));
                builder.line_to(Point::new(w, h));
                builder.line_to(Point::new(cut, h));
                builder.line_to(Point::new(0.0, h - cut));
                builder.close();
            });
...
/// A container with a custom Cyberpunk-themed info panel (diagonal cuts).
pub fn info_panel<'a, Message: 'static>(
    content: impl Into<Element<'a, Message>>,
    color_accent: Color,
    color_bg: Color,
) -> Element<'a, Message> {
    let bg_program = InfoPanelBackground {
        bg_color: Color { a: 0.15, ..color_bg },
        border_color: color_accent,
        cut_size: 15.0,
        border_width: 1.5,
