...
///
/// Selected is not a fifth role. In the kitsch target the band on the
/// solid-yellow card is the bottom stop of the same gradient the card
/// is filled with -- a shade of itself, not another colour -- because a
/// band in the selection fill on a card in the selection fill is
/// invisible. We render selection flat, so the equivalent move is to
/// pull the fill a little way towards its own ink.
