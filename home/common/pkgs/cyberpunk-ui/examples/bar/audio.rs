...
//! `None`, which [`cyberpunk_ui::bar`] draws as no module at all rather
//! than as a volume of zero it cannot vouch for.

use crate::sensor::{Latest, Snapshot};
use cyberpunk_ui::bar::Audio;
...
impl Monitor {
    ...
    pub fn spawn() -> Self {
        ...
        let _ = thread::Builder::new()
            .name("cyberpunk-bar-audio".to_string())
...
fn session(shared: &Snapshot<Option<Audio>>) -> bool {
    ...
    let _ = proplist.set_str(properties::APPLICATION_NAME, "cyberpunk-ui-bar");
    let _ = proplist.set_str(properties::APPLICATION_ID, "cyberpunk-ui-bar");

    let context = {
        let ml = mainloop.borrow();
        match Context::new_with_proplist(&*ml, "cyberpunk-ui-bar", &proplist) {
