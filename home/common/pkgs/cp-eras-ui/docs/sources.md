...
| SVG | traced from |
|---|---|
| ~~`docs/neomil/dashboard.svg`~~ | **Deleted 2026-09-03.** The app-shaped composite (three red chart cards, a services table) was credited to `img-07-dashboard.png` but shares only its palette and header grammar with it. `dashboard-trace.svg` below is the design; `src/screens/dashboard.rs` still draws the composite's layout until the `Layout` decision in TODO.md. |
| `docs/neomil/dashboard-trace.svg` | `images/img-07-dashboard.png` — **G1 schematic** of the photo, rewritten 2026-09-01 from measured geometry (`scripts/extract_spec.py`): a broad blue glow over near-black that is gone by y~420 (not a band with an edge); a header of CUSTOMER + one chamfered LEVEL badge, the "next TECHNOLOGY" logotype, and SECURITY LEVEL + four chamfered LEVEL badges with the second filled; a hairline rule at y=187; a tab row (COMPUTER SYSTEMS, DESCRIPTION); **a six-diamond staggered menu**, half-diagonal 104, centres (334,460) (530,460) (725,460) / (431,593) (628,592) (822,592), labelled above row 1 and below row 2, **every outward tip cut flat** — a 30px plateau at 89 from the centre (row 1 tops at y 371, row 2 bottoms at y 682), inner outline flat at 59, two explicit defs `#cell-up`/`#cell-down` (re-measured 2026-09-03 late; the trace had drawn full tips); a chamfered GO HOME info panel at x 1128..1366, y 314..756 (chamfers top-right 8 and bottom-left 42, a filled edge bar x 1358..1366 y 405..516 chamfered 8 at both ends (bottom (1366,508)→(1358,516), corrected 2026-09-03 late from "nearly flat" 512/515), **no scrollbar** — corrected 2026-09-03 from the trace itself, which had said so all along); rotated micro-text down both margins and a two-cell **outlined** footer tape x 1209..1354 (1px frame, divider at x 1270, code left, two micro-text lines right; corrected 2026-09-03 late from two filled cells + one outlined, which were the extractor's blobs). Gate re-run 2026-09-03 late: PASS 94% held, diamonds 12/12. |
