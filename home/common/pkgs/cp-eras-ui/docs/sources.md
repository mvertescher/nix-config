...
four contiguous era runs, each with a near-black title card followed
by nine screens:
...
## Neo-militarism (title #53, screens #54–62)

The local `53-5707b711.png` thumbnail is visibly the NEO MILITARISM /
SUBSTANCE OVER STYLE title card; the three-card login is #59
(`59-6cfb2011.png`). Rechecked 2026-09-14: the earlier claim that `5707b7`
was the login was incorrect.
...
| SVG | traced from |
|---|---|
| ~~`docs/neomil/dashboard.svg`~~ | **Deleted 2026-09-03.** The app-shaped composite (three red chart cards, a services table) was credited to `img-07-dashboard.png` but shares only its palette and header grammar with it. `dashboard-trace.svg` below is the design; `src/screens/dashboard.rs` still draws the composite's layout until the `Layout` decision in [the pipeline record](../todo/design-pipeline.md). |
| `docs/neomil/dashboard-trace.svg` | `images/img-07-dashboard.png` (#60, 3840×2160) — measured schematic in a 1600×900 frame. Six staggered menu tiles retain 104px half-diagonals and flattened outward tips at 89; each has a fitted inset with a side cut, 1px outline, solid edge tab and distinct matrix/code/symbol printing. GO HOME restores nine actual lines, vertical brands, local scanlines and horizontal field variation; connected, scan-modulated contour/bar echoes replace four exterior rectangles. Header and secondary lettering use compact source vectors, including KIROSHI's curved inner O arc; its earlier slash reading was too strong. Badge fields have measured gradients/stripes, DESCRIPTION is square, and menu labels use Medium 20.625. A sampled composite restores the dashboard background while keeping the main tile red flat. Six label and sixteen header printing groups now have locally measured softened copies; tape/chip faces use independently sampled #fb3535, and tape-code/chip-1 ink has measured periodic variation. Five tape/chip silhouettes, nine GO HOME body lines and the maker shape also have locally validated echoes. The reference app now maps its dashboard foreground to source/SVG #ef3333 while preserving custom palettes, other screens and fixed interaction colors. Matrix/margin printing, GO HOME heading, maker primary contour/ink/microtext, vertical-brand echoes, chip-2 ink and the tiny tape mark remain open. Fine background patches are identical across all four sources; the original material/authoring recipe and echo origin remain unresolved. The detail batch passes 209 Rust tests and both fidelity gates with reviewed app/state captures; all 19 repository checks pass, including all 25 golden cases. See [the fidelity report](neomil/dashboard-fidelity.md) and [dashboard TODO](../todo/neomil-dashboard.md). The earlier 94% shape-area PASS described broad layout, not detail completeness. |
| `docs/neomil/login-trace.svg` | `images/img-06-private.png` (#59) — **G1 schematic**, 2026-09-02: the hub's header (CUSTOMER / LEVEL T1 badge, #NC488402 block, SECURITY LEVEL T1–T4 with T2 filled) over a hairline rule; three USER 01 cards at x 372..625 / 692..947 / 982..1236 (card 1 y 313..572 plus its password field and Login bar to y 683; cards 2 and 3 y 315..655) with a 51px top-right chamfer and a 15px-deep notch in the left edge at y 390..496, ends cut 15 top / 13 bottom (corrected 2026-09-03 twice: first to the trace's own paths, this row had 375..637 / 700..962 / 987..1250, y 312..675 and a 13px chamfer; then the same day the paths themselves were re-measured against the photo, chamfer 46/47 → 51 and notch 392..497 → 390..496) — card 1 solid red with a chip glyph, cards 2 and 3 dark translucent with a portrait and a CC35 micro-caption; under card 1 a `password:` label, a dotted field and a `Login` bar, both outlined red; rotated micro-text in both margins. Gate: PASS, 79% area (the source's four "diamonds" are the extractor fitting the portraits' faces and a card-fill fragment; below the class-share threshold). |
| `docs/neomil/mailbox-trace.svg` | `images/img-08-main.png` (#61) — **G1 schematic**, 2026-09-02: the same header plus a COMPUTER SYSTEMS / CONTENT tab row; a column of eight disc icons at x~100 with a message list beside it, rows `List of messages / Jackie` (first, filled red, with a NEW pill) then I'm worried man / Heist data sent to you alternating, three rows flagged NEW; the `Urgent Information (!)` heading over an outlined chamfered message panel x 729..1450, y 312..699 (8px chamfers) with four lorem paragraphs; a button row at y 702..758 of four 175-wide cells 5 apart (corrected 2026-09-03 to the trace's paths; this row had x 940..1255, y 315..755 and buttons at y 760..790), `Switch Weapon` filled and three `Confirm / Jump` outlined; a small R/arrows glyph stack between list and panel. Gate: PASS, 86% area. |
| `docs/neomil/store-trace.svg` | `images/img-09-store.png` (#62) — **G1 schematic**, 2026-09-02: NO header row — a KIROSHI chip strip at y 31..47, the MASURAO logotype (kanji 益荒男 over a slanted MASURAO band) top-left, a filled CUSTOMER #NC488402 bar, LOYALTY DISCOUNT / LAST UPDATE lines, a five-row nav (VIDEO filled, AUDIO, GAMEPLAY, CYBERWARE, CONTROLLER) with a 16px bottom-left chamfer and a spine; four MAGNUM 650 HAND GUN cards at x 437 / 769 / 1096 / 1425, tops y 151, bottoms y 613, each with a bright right-edge bar y 266..416, three head/foot icons, a pistol drawing, DPS PNT ACC ROF over 86 30 5 5, a socket row; card 2 selected — taller (to y 800), lighter upper fill, solid pistol, 20 RECOIL / 22 SPERAD / 12 RANGE, BONUS +9 REFLEXES / +2 MODULES SLOTS, a second socket row; card 4 cut by the frame at x 1557 (corrected 2026-09-03; the row said x~1555 while its own last sentence said 1557). Gate: PASS, 71% area (second pass 2026-09-02; was FAIL 55 with the rifles as silhouettes; not re-run 2026-09-03 — the trace header records no gate number). The rifles are now four layered symbols measured from 10x gridded crops — body, hatched forend, highlights, detail lines — card 2's solid and 14px further left as in the photo; the kanji is real Noto CJK text on the slanted band; card 4 ends in a plain cut at x 1557 (the chamfer and right-edge bar there were invented) and its socket glyph is the source's 24-cell scatter, not a finder-square QR. Only card 4 ever had a clipPath (fixed earlier the same day). |
| ~~`docs/neomil/target-app.svg`~~ | **Deleted 2026-09-03**, superseded by `mailbox-trace.svg` and `store-trace.svg`. It was an original composition despite its "traced from `img-08-main.png`" claim: opened side by side 2026-09-02, the photo is a message list beside an "Urgent Information (!)" panel, the SVG a NEOMIL OPS services table + sessiond panel sharing only the palette and the header grammar. `src/widgets/table.rs` and `style::Menu::Table` were built to it and say so. |
| ~~`docs/neomil/target-components.svg`~~ | **Deleted 2026-09-03.** Claimed to be "sampled across the run" but sampled nothing from it — every component was the composites' invention. Replaced the same day by `components.svg` below. |
| `docs/neomil/components.svg` | **Widget sheet, built 2026-09-03 from the four neomil traces + `bar.svg`** — every component is a translated copy of a trace element (no scaling), cited to file and coordinates in an XML comment and captioned with its measurements on-sheet: header chrome (CUSTOMER/LEVEL badge, SECURITY LEVEL row, #NC488402 block, `next` and MASURAO logotypes, tab boxes, footers, margin chips), nav cell both states, message-list rows (selected / NEW / plain) with disc column and scroll widget, menu diamond + separator, buttons, NEW pill, user card active and idle, product card plain and selected, GO HOME panel, message-panel corner, 36 measured palette values, typography, ground stops, era rules and an implementation-delta box. Not a photo source and not gated; the traces are. |
...
| SVG | traced from |
|---|---|
| ~~`docs/entropism/dashboard.svg`~~ | **Deleted 2026-09-03.** The app-shaped composite (a single row of four tiles) was credited to `images/entropism-dashboard.png` — which is the store (see the name-swap warning). `dashboard-trace.svg` below is the design. |
| `docs/entropism/dashboard-trace.svg` | `images/entropism-store.png` (the hub — see the name-swap warning) — **G1 schematic**, rewritten 2026-09-01 from measured geometry the way the neomil one was; gated by `fidelity_check.sh --inventory entropism dashboard` (shapes mode, 92% area). The previous file was a fabrication that named the wrong source and concluded the hub's grid, sidebar and detail panel "are not in the frame"; the row above it saying so is retired. Checked by eye 2026-09-02; footer fixed the same day: only BUILD is right-anchored (text-anchor end at 1525), PROVIDED BY is left-anchored at 518 as on the mailbox — the trace had it 180px too far right. The header strip strings were ~8% short and ~40px left (font-size 14 vs the mailbox's 15); fixed 2026-09-03 to 17/500 at x 61/518/1382, baseline 60, footer to match — no longer open. Gate 92% not re-run 2026-09-03 (the trace header records no gate number). (corrected 2026-09-03) |
| `docs/entropism/login-trace.svg` | `images/entropism-login.png` (#39) — **G1 schematic**, 2026-09-02: the outlined header strip x 49..1547, y 43..69 with dividers at 465/1353; an empty upper two-thirds; `USERNAME:` (baseline 402 — corrected 2026-09-03 from 403; the trace's text sits at y 402 and its header prose still says 403) over an outlined field x 563..922, y 414..447 of eleven asterisks with a caret underline, and a solid NEXT button x 932..1037; a tall solid sage footer band x 36..1565, y 765..880 (12% of the frame) carrying dark INTERFACE LOADED / PROVIDED BY NEXUS NETWORK V10.8 / BUILD 6.47.48441.R15 — no outline box, unlike the other screens' thin footer strip. Gate: PASS, 83% area (the source's ten "chamfers" are header letterforms; below the class-share threshold; not re-run 2026-09-03 — the trace header records no gate number). |
| `docs/entropism/mailbox-trace.svg` | `images/entropism-mail.png` (#41) — **G1 schematic**, 2026-09-02: header strip; A MAIL BOX / B MESSAGE / C ENCRIPTION LEVEL letter boxes at y 98..124; the list outlined x 84..451, y 205..686, seven rows on a 62px pitch, row 1 (YOU'LL REGRET THAT / FROM: JACKIE) filled solid, envelope glyph per row, titles URGENT INFORMATION (!) / HEIST DATA SENT TO YOU / I'M WORRIED MAN / SPECIAL OFFER TO YOU! / I'M WORRIED MAN / SPECIAL OFFER TO YOU! with FROM: MOM / 805000451 / RACHEL ROSS / JINX JINX STORE / BIALA ROBERTSON / LARIX & BETULA; the MESSAGE panel x 529..1279, y 205..686 with a filled title bar y 227..288 over three lorem paragraphs; a button row y 694..746, REPLY / FORWARD / DELETE outlined and REPORT SPAM filled; 2x2 badges of 69px at x 1341/1418, y 227/305 reading T1 T3 / T2 T4 with T2 filled; outlined footer strip y 847..873. Gate: PASS, 88% area (second pass 2026-09-02; was FAIL 60; not re-run 2026-09-03 — the trace header records no gate number). Every outlined frame in the material was read at 1600 as a 2px stroke with a 1px near-black undershoot and a faint sage overshoot 2–4px out; drawn as three concentric strokes per section (`#25281d` 7px / `#0a0a02` 4px / `#709174` 2px) and recorded in the header as a photographic halo, not a designed glow — the iced side draws only the 2px stroke. Corrected 2026-09-03: probed at the full 3840 resolution the undershoot is NOT in the photo — the stroke there is a 3px bright line (mean core `#8fba97`) sitting straight on the dark ground; the dark ring is the negative lobe of the Lanczos filter used for the 1600 rescale; the sage overshoot is real. The 7px/4px passes now carry `class="photo"` (hidden by G2i; G1i still renders them) and the design stroke stays the 2px `#709174`. Envelope glyphs rebuilt from 10x crops (closed 17x11 with flap V, open 17x16 with a diamond flap) at the measured row tops. The "double-fitted frame" was actually a text-halo blob in rows 2..7 bounded by the selected row; the added ring pairs with it by bbox coincidence. |
| `docs/entropism/store-trace.svg` | `images/entropism-dashboard.png` (#42, the store despite the name) — **G1 schematic**, 2026-09-02: header strip (DIGITAL DISTRIBUTION SOFTWAREV2); the 4ST logotype x 134..308, y 102..160 with 4S solid and T outlined, S T O R E beneath; an outlined CUSTOMER / #NC488402 box and LOYALTY DISCOUNT / LAST UPDATE lines; the nav x 112..330, y 301..740 — RIFLES, SMG (filled), SNIPER, SHOTGUN, PISTOL, then one tall empty cell; four MAGNUM 650 HAND GUN cards 265 wide on a 322 pitch at x 461/783/1105/1429 (the fourth clipped), each with a yellow PETROCHEM / BETTERLIFE TEC band y 315..335, a rifle, DPS/PNT/ACC/ROF over a filled 86/30/5/5 row y 469..494, a socket row and a CC35 micro-caption; the **first** card grown, header filled through the values row and continuing to y 720 with 20 Recoil / 22 Sperad / 12 Range, Bonus / +9 Reflexes / +2 Modules Slots; A and B letter boxes at y 779..805; outlined footer strip. Supersedes `target-app.svg`. Gate: PASS, 80% area (not re-run 2026-09-03 — the trace header records no gate number). |
| ~~`docs/entropism/target-app.svg`~~ | **Deleted 2026-09-03**, superseded by `store-trace.svg`; the store screen is built from the trace. It was a loose composite of the right screen: it grew the second card where the photo grows the first, had no pale header blocks and no PETROCHEM band, and drew 228-wide cards on a 250 pitch all on canvas. |
| `docs/entropism/components.svg` | **Widget sheet, rebuilt 2026-09-03 from the four entropism traces + `bar.svg`** (was `target-components.svg`, a by-eye sheet whose every dimension the traces contradicted — 120x120 tiles vs 194x212, second card grown vs first, no PETROCHEM band, no socket row). Translate-only copies of trace elements, each cited and captioned: header/footer strips, login footer band, letter boxes, mail-list rows, envelope glyphs, category nav, hub tile both states, product card plain and grown, socket row and QR glyph, 4ST logotype, message panel + button row, hub detail panel, security badges, login field + NEXT, palette, typography, ground, era rules, implementation delta. Not gated; the traces are. |
...
| `images/kitsch-store.png` | `75b8de118663901.60e5fa669b49a.png` (#52) | **store** — see the `store-trace.svg` row |

| SVG | traced from |
|---|---|
| `docs/kitsch/dashboard-trace.svg` | `images/kitsch-dashboard.png` — **G1 schematic**, written 2026-09-01 from measured geometry (PCA on the ink masks): two 3-blade fans of 162x50 r8 cards (the 190x60 r14 first drawn was wrong; re-measured 2026-09-02 in each card's own frame — ghosts are the same size as the solid card, stepping (+20,−20) in screen space, counts 6/7/6/6/5/6, fills fading 0.58→0.12), hubs ~(455,470) and ~(825,535), EVENTS solid yellow, ghost stacks per blade that all recede up-right (-45 deg, the (+20,−20) step) in screen space — one shared depth direction, not each card's normal (fixed 2026-09-02; the previous per-normal version sent WEAPONS and EVENTS up-left and still scored 0.47); USER box, 01–04 badges, BRAINDANCE panel, A/B/C/D letter boxes. Gated by ink placement (`fidelity_check.sh --inventory kitsch dashboard`): faithful 0.69 per the trace's 2026-09-03 pass note (0.59 on 2026-09-02; 0.54 before the card resize and the mint footer — it had been drawn yellow) vs 0.07 for the old app composite. (corrected 2026-09-03: angle was written ~-50 deg, gate 0.59) |
| `docs/kitsch/login-trace.svg` | `images/kitsch-login.png` (#50) — **G1 schematic**, 2026-09-02: rose bloom top-centre gone by y~420; 10:20 PM at x 781..852; a 1.3px mint bracket (corrected 2026-09-03 from "2px"; the trace measured 1.2 and draws 1.3) running the full frame height (fading into the bloom above y~130), left x 228.5, right x 611.5, whose left edge breaks at y 540 into a ~57° diagonal to (338,635), drops to 700 and rounds (r~30) into a bottom at y 731, with a dark-teal lobe outside the diagonal; three GUEST 7702 cards on a 393px pitch (x 258/651/1043) — 62x61 chip glyph, name, boxed A + ACCESS MANAGER micro-text; card 1 has a dark input field [257,413,335,51] and a solid mint ENTER bar y 470..497 stepping up at its right, cards 2/3 the same bar in dark teal reading PROTECTED; a barcode in the bracket foot x 377..590, y 632..683 with digits 12345678123456789; ARASAKA footer micro-text. Gate: PASS, inks 0.63 per the trace's 2026-09-03 pass note (0.54 on 2026-09-02, 0.47 first pass; barcode regenerated from the column profile to 50 bars, bracket 1.3px as measured, chip glyph drawn, footer to the shared mint size 9). |
| `docs/kitsch/mailbox-trace.svg` | `images/kitsch-mail.png` (#51) — **G1 schematic**, 2026-09-02: same bloom; boxed A/B/C at x 165/586/1217 (corrected 2026-09-03 from 164/586/1216; rects at 165.2/586.2/1216.6), labels USER / DESCRIPTION / SECURITY LEVEL, notched boxes GUES 7702 and MAILBOX (y 185..236), four 56x34 badges with 02 filled orange; a left bracket outline from x 120.5, y 268 down to a solid teal wave y 606..748 sweeping right to x~376; five rows on a 60px pitch — YOU'LL REGRET THAT / Jackie selected as a yellow icon cell + chamfered body y 313..351, then URGENT INFORMATION (!) / Mom, HEIST DATA SENT TO YOU / 805000451, I'M WORRIED MAN / Rachel Ross, SPECIAL OFFER TO YOU! / JINX JINX STORE; a solid yellow message tab x 575..1127, y 313..349 over a yellow-outlined body to y 748 with a top-left notch and ten lorem lines; four chevrons at x 1216 (DETAILS solid yellow; MODS / PRICE / DAMAGE teal outline). Gate: PASS, inks 0.67 per the trace's 2026-09-03 pass note (0.62 on 2026-09-02, 0.46 first pass; body text 16.8/500 to the measured line extents, panel outline 1.25px on .5 coordinates r8, footer fixed). |
| `docs/kitsch/store-trace.svg` | `images/kitsch-store.png` (#52) — **G1 schematic**, 2026-09-02: same bloom; 4S solid + outlined T logotype (x 151..318 — corrected 2026-09-03 from 155; the trace measures 150.8..317.5), S T O R E beneath; a rounded customer chip x 123..338, y 178..200 and loyalty/last-update lines; the nav bracket — top line y 186 S-bending round the customer block to y 268, r~30 corner into a left edge at x 106.5 that becomes a solid teal wave at y~580 (top edge y 617 to x 312, shoulder to x 345, bottom line y 718 fading by x~445); five 216x39 peaked chevrons at x 140 on a 60px pitch from y 297 (RIFLES, SMG solid #ffbe18, SNIPER, SHOTGUN, PISTOL); four 261-wide cards on a 320 pitch (x 484/804/1123/1443, fourth clipped), teal outline y 218..538 with a 24px top-right chamfer, MAGNUM 650 / HAND GUN, a solid yellow band y 277..312 with a 27px flag past the left edge, a mint gun, DPS PNT ACC ROF, a solid mint values bar y 450..480, a socket row y 492..538; card 2 amber-filled to a stepped bottom then amber-outlined to y 680 with 20 Recoil / 22 Sperad / 12 Range, Bonus / +9 Reflexes / +2 Modules Slots and a second socket row; boxed A at x 354 and C at x 1501 in the foot (corrected 2026-09-03 from 350/1500). Supersedes `target-app.svg`. Gate: PASS, inks 0.73 per the trace's 2026-09-03 pass note (0.57 on 2026-09-02, 0.55 first pass; gun silhouette from the mint-mask column profiles with evenodd holes, card 2's gun 14px higher as in the photo, band glyphs redrawn as distinct marks at measured positions). |
| ~~`docs/kitsch/dashboard.svg`~~ | **Deleted 2026-09-03.** Original composite drawn before the hub was found: its fan widget came from the fan scenes but its chrome matched the app, not `kitsch-dashboard.png`. Scored 0.07 on the ink gate. `dashboard-trace.svg` above is the design. |
| ~~`docs/kitsch/target-app.svg`~~ | **Deleted 2026-09-03**, superseded by `store-trace.svg`; the store screen is built from the trace. The closest of the four `target-app` composites (composition and grown card right), but its nav was 160-wide rounded rects where the photo has 216x39 chevrons inside a bracket ending in the teal wave, and its yellow band was ~20px with no flag notch. `src/style.rs` `Ticket` and `Banner` still carry its pill and 20px-band numbers — the open kitsch item in TODO.md. |
| `docs/kitsch/components.svg` | **Widget sheet, rebuilt 2026-09-03 from the four kitsch traces + `bar.svg`** (was `target-components.svg`, by-eye: 160-wide pills, 20px bands with no flag, extruded slabs, an orange device bezel no traced screen has). Translate-only copies of trace elements, each cited and captioned: 4ST logotype, customer chip, letter boxes, stepped USER box, 01–04 badges, the full nav container (bracket + wave + five 216x39 chevrons), chevrons and tabs isolated in both states, fan blade + ghost stack idle and selected, mail rows, product card plain and grown, BRAINDANCE panel, message panel, guest card, input field, ENTER/PROTECTED bars, barcode, socket glyph, palette, typography, ground, era rules, implementation delta. Not gated; the traces are. |
...
| `images/neokitsch-store.png` | `ca9fd2118663901.60901b230b10d.png` (#72) | **store** — see the `store-trace.svg` row |

| SVG | traced from |
|---|---|
| `docs/neokitsch/dashboard-trace.svg` | `images/neokitsch-dashboard.png` — **G1 schematic**, written 2026-09-01 from measured geometry: six 91.7x328 cards (top-right chamfer 42.5, one 45° diagonal across the whole foot, a 6x38 solid plate on the left edge) in staircase triplets at (246,384)(347,284)(449,182) and (624,384)(724,284)(826,182), EMAIL solid gold with a plate well and no rings, the five unselected cards each carrying six onion outlines nested inside the card, sharing its left edge and foot (re-measured against the photo 2026-09-03: this row had "93x327, chamfer top-right + bottom-left, concentric outward rings"; the old `#ncard` was an 83 body plus a 10 tab well with a 32 chamfer, and the rings were offset outward); detail panel at (1170.8,259.7): 230.4x465.3 with a stepped top edge (shoulder at the left climbing through an S-curve to the top line), four onion rings nested inside, veneer body [1170.8,326,230.4,309] carrying two paragraphs of text, a two-line micro-text tape and the EMAIL label beneath (re-measured 2026-09-03 late; this row had "(1168,253)" and the trace had the cascade card mirrored with a 42 chamfer and six rings offset outward, none of it in the photo); wire band, letter boxes, T1–T4 badges. Gated by ink placement: 0.60 (0.51–0.54 before the halo filter shared with the three newer traces was ported in, 2026-09-02; re-run 2026-09-03: 0.67 after the card, 0.68 after the panel; 2026-09-07: 0.64 after the veneer grain went onto EMAIL's card and the panel body and the paragraph, caption and tape bars became Rajdhani text, the panel copy the inbox's selected message. The grain is what the photo shows on both fills, at the sampled pitches with the clipped mid-gold strokes the store and mailbox traces use, and the drop is k-means re-binning the dark family from bars to glyphs, not placement) vs 0.03 for the old app composite. |
| `docs/neokitsch/login-trace.svg` | `images/neokitsch-login.png` (#70) — **G1 schematic**, 2026-09-02: the run's haze, retuned to this photo 2026-09-03 (#haze cx 770 r 1000 y-scale 0.49, no longer the hub's cx 825 r 1030 0.515); ARASAKA stencil logotype x 92..272, y 59..88 (corrected 2026-09-03 from 93..278; the trace measures 92.1..272.1) with tagline and a two-cell outlined box (57ASD4AV15AA / COMBAT COLONIZATION – DEFENCE PROGRAM); 10:10 PM / NIGHT CITY / AREA top-right; two identical entry groups (second at +420 x): PRASE_6054012 label, an unoutlined chocolate field [417,361]..[762,403], a solid gold ENTER / LOGIN bar [417,414]..[761,441] with a 16x13 bottom-left cut, letter box A/B with micro-text; a 22-strand wire band as a wide trapezoid across the foot (outer plateaus y 727..809, S-bends mirrored about x=808 onto a centre plateau y 782..846), ends curling down at x 35/1564; centred footer line at y 875. Nothing else. Gate: PASS, inks 0.77 per the trace's 2026-09-03 pass notes (0.73 on 2026-09-02). |
| `docs/neokitsch/mailbox-trace.svg` | `images/neokitsch-mail.png` (#71) — **G1 schematic**, 2026-09-02: the hub's header block verbatim (mean abs diff 2.7/255 against the hub photo); seven message rows on a 60.2 pitch from y 309 (title, FROM line, open/closed envelope glyph at x 429, hairline rule with a small tab at x 421..452), row 2 the selection as a solid gold plate [35,315]..[512,370] with a chamfered right end and a notched bottom edge at x 419..458 (gold tab hanging to y 372 under a dark trapezoid outline to y 366; the header described it from 2026-09-02 but no path drew it until 2026-09-03); the message as plain text with no panel — URGENT INFORMATION (!) bold, FROM: MOM, three lorem paragraphs; four outlined RIFLES buttons 184x39 at y 684 on a 192 pitch with a bottom-left chamfer and a filled tab; letter boxes C (139,777) / D (735,777). Every glyph sits on a soft vertical glow, modelled as a blur filter. Gate: PASS, inks 0.73 per the trace's 2026-09-03 pass notes (0.68 on 2026-09-02; was FAIL 0.61 with #c19867 unpaired). The source's gold is three-tiered — bright bar/tabs, mid text cores, dark outlines — and the first trace was one flat bright gold; the selection bar is now wood veneer (32 fine strokes, a seam at x 273), RIFLES outlines take the dark gold, list titles the mid gold, and all four source families pair (0.78 / 0.86 / 0.53 / 0.33 on 2026-09-02; #c19867 rose to 0.44 on 2026-09-03). The halo that was ~2.4x too abundant around body text (11594 glow pixels vs the source's 5644) was fixed 2026-09-03 by splitting the content into #lines / #text with a tighter #halotext filter (now 5648) — no longer open. (corrected 2026-09-03) |
| `docs/neokitsch/store-trace.svg` | `images/neokitsch-store.png` (#72) — **G1 schematic**, 2026-09-02: 4S solid + T outlined logotype (x 109..277, y 66..117), S T O R E beneath; a BASKET plate [1292,20]..[1496,105] split by a hairline at y 60; an 8-strand header band rising through one S-bend (290→349, mirrored) onto a bridge line y 124 spanning 349..1230, letter boxes A (360,143) and C (1178,143); CUSTOMER / LOYALTY DISCOUNT / LAST UPDATE lines, then five outlined 200x39 buttons on a 60.7 pitch from y 358 (RIFLES, SMG solid gold, SNIPER, SHOTGUN, PISTOL) with a bottom-left cut and a solid tab; four 262-wide cards (x0 360.8 / 667.1 / 978.8 / 1288.8) with an r13 top-left, a 37° step up to a r18 top-right, and four fading echo strands round the top-right, side and bottom — rifle, DPS PNT ACC ROF / 620 30 5 5, socket row, MAGNUM 650 HAND GUN, a tab under the bottom edge; card 2 expanded (top −82, bottom +71) and solid gold across y 411..653 carrying 20 Recoil / 22 Sperad / 12 Range / Bonus / +9 Reflexes / +2 Modules Slots in dark text; letter box B (675,775). Supersedes `target-app.svg`. Gate: PASS, inks 0.66 per the trace's 2026-09-03 pass note (0.60 on 2026-09-02 — the haze retune alone dropped it to 0.46 before the #lines/#text halo split; 0.63 first pass; wood-grain strokes added on the SMG, card 2 and basket at the measured ~2.1px pitch and seams — k-means re-partitioned and the glow family fell 0.87→0.70, accepted because the overlay is closer). |
| ~~`docs/neokitsch/dashboard.svg`~~ | **Deleted 2026-09-03.** Original composite whose cascade widget was credited to `7b4675…` (actually a login screen); the real hub went unread. Scored 0.03 on the ink gate. `dashboard-trace.svg` above is the design. |
| ~~`docs/neokitsch/target-app.svg`~~ | **Deleted 2026-09-03**, superseded by `store-trace.svg`; the store screen is built from the trace. Right screen, but it invented a full-screen chamfered device frame the photo does not have (`widgets::chrome::DeviceFrame` was built to it and is now only worn by the bar window), lacked the onion outlines that are the era's signature, drew 200-wide cards where the photo has ~265, and borrowed kitsch's "ARASAKA CONSUMER TECHNOLOGY" footer. |
| `docs/neokitsch/components.svg` | **Widget sheet, rebuilt 2026-09-03 from the four neokitsch traces + `bar.svg`** (was `target-components.svg`, by-eye: a device-frame miniature, strata divider, step-notch pills and 150x24 nav pills, none in any trace). Verbatim-spliced trace geometry, translated only, each cited and captioned: ARASAKA and 4ST logotypes, clock block, CUSTOMER/LEVEL, SECURITY LEVEL with the ringed T2 folder badge, BASKET plate with grain and QR, letter box, cascade card pair both states with onion rings, store nav cell plain and veneer-selected, mailbox RIFLES button, product card plain and expanded (66 grain paths), login entry group, hub detail panel, message panel, mail rows plain and veneer-selected, three 300px windows onto the wire bands, store meta lines, 32 palette values, typography, ground, era rules, implementation delta. Not gated; the traces are. |
...
modes were calibrated against a known-wrong drawing — the app-style
`dashboard.svg` composites, deleted 2026-09-03; the "wrong drawing"
column records their scores so the gate's floor stays on record:
...
| kitsch | `kitsch-dashboard.png` | inks | **PASS**, weighted IoU 0.69 (0.59 after the 2026-09-02 card resize, 0.54 before; third pass 2026-09-03) | 0.07 |
| neokitsch | `neokitsch-dashboard.png` | inks | **PASS**, weighted IoU 0.68 (0.67 before the detail panel was re-measured 2026-09-03 late; 0.60 before the cascade card was re-measured 2026-09-03; 0.54 before the halo was added, 2026-09-02) | 0.03 |
...
| kitsch (inks, IoU) | PASS 0.63 (was 0.54, 0.47) | PASS 0.69 (was 0.59, 0.54) | PASS 0.67 (was 0.62, 0.46) | PASS 0.73 (was 0.57, 0.55) |
| neokitsch (inks, IoU) | PASS 0.77 (was 0.73) | PASS 0.68 (was 0.67, 0.60, 0.54) | PASS 0.73 (was 0.68, FAIL 0.61) | PASS 0.66 (was 0.60, 0.63) |
