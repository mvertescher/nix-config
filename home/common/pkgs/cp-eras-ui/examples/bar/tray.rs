...
//! already has -- see `cp_eras_ui::bar::tray_menu` for why that is
...
use cp_eras_ui::bar::{MenuEntry, MenuKind, TrayAction, TrayItem, TrayMenu};
...
pub struct Config {
    ...
    /// The size icons are drawn at, from `cp_eras_ui::bar::icon_size`,
    /// so that what is decoded matches what is drawn.
    pub icon_size: u32,
    /// The same for a menu row's icon, from
    /// `cp_eras_ui::bar::menu_icon_size`. A separate number because a
