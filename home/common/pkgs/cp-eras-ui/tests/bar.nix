...
# implements no `wlr-layer-shell` -- so `cp-eras-ui-bar` would come up
...
# one.
#
# So this case renders `cp-eras-ui-bar-window` instead: the same
...
#     reading. Those are properties of `cp-eras-ui-bar`, and only a
