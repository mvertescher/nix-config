...
//
// That original set shrank from seven modules to two; the shared modules
// above serve the current callers. The rule that emptied the old set is
// worth keeping: a widget that draws *one era's* geometry and has
...
//     `row::mail_row` initially replaced its pose (that module was later
//     deleted too). The working client's clickable row now lives in
//     `panels::mail`, using `surface` feedback widgets. The original
//     deletion removed 213 lines without moving a golden pixel.
...
//     was credited with moved to layout widgets. Today `panels::mail`
//     uses `surface::{interactive, cursor_list}`, `bar` uses `mouse_area`,
//     and the trace-driven screens use `screens::scene` hit-testing.
...
//     were assembled from. Deleted 2026-09-05 after the reference screens
//     became `Prim` tables: not one of the eight had a caller outside
//     the others. The working mail client still uses layout widgets.
//     What the roadmap still wants of them is rebuilt from
